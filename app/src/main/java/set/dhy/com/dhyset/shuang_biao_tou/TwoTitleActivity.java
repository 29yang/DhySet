...
public class TwoTitleActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_two_title);
        //初始化组件
        initComponent();
        //使用本的数据
        initData();
    ...
    private LinearLayout getDataColumn(final int platformIndex) {
        ...
        final int rowCount = timeList.size();
        //需要合并的行数
        long mergeRow = 0;
        for (int j = 0; j < rowCount; j++) {
            ...
            final Map<String, String> pInfo = dataList.get(platformIndex);
            int dealState = -1;
            for (int i = 0; i < selectedList.size(); i++) {
                ...
                if ((selectedMap.get("pid")).equals(pInfo.get("pid"))) {
                    ...
                    if (tId == startTid) {
                        ...
                        tag.add(selectedMap.get("pid"));
                        tag.add(timeSpinnerMap.get("tid"));
                        textView.setTag(tag);
            ...
            textView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    List<String> tag = (ArrayList<String>) textView.getTag();
                    if (tag != null) {
                        for (int i = 0; i < selectedList.size(); i++) {
                            if (selectedList.get(i).get("pid").equals(tag.get(0)) && selectedList.get(i).get("start_tid").equals(tag.get(1))) {
                        ...
                        //合并了3行
                        Map<String, String> map = new HashMap<String, String>();
                        map.put("pid", String.valueOf(platformIndex));
                        map.put("start_tid", String.valueOf(finalJ));
                        map.put("end_tid", String.valueOf(finalJ + 2));
                        map.put("status", String.valueOf(STATUS_ONE));
