...
//                    setTopView();
    ...
    private void layoutChildren(int startingIndex, int adapterCount) {
        while (startingIndex < Math.min(adapterCount, MAX_VISIBLE)) {
            ...
            Log.e("=====",newUnderChild.toString()+"");
            if (newUnderChild.getVisibility() != GONE) {
                makeAndAddView(newUnderChild, startingIndex, Math.min(adapterCount, MAX_VISIBLE));
    ...
    private void makeAndAddView(View child, int startingIndex, int len) {
        ...
        float v;
        if (len < MAX_VISIBLE) {
            v = startingIndex * 30 - 30 * scale;
        } else {
            if (startingIndex == len - 1) {
                v = (startingIndex - 1) * 30;
            } else {
                v = startingIndex * 30 - 30 * scale;
            }
        }
        ...
        childTop += v;
        child.layout((int) (childLeft + v / 2), childTop, (int) (childLeft + w - v / 2), childTop + h);
    }
